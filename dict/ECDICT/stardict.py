...
class DictMySQL (object):
    ...
    def __open (self, force_init=None):
        mysql_startup()
        if MySQLdb is None:
        ...
        # 如果force_init为None，使用原来的逻辑；如果为False，强制不初始化
        should_init = self.__init if force_init is None else force_init
        if not should_init:
            uri = {}
    ...
    # 初始化数据库与表格
    def init (self):
        database = self.__argv.get('db', 'enreading')
    ...
    def commit (self):
        try:
            ...
            if "Already closed" in str(e) or "'Connection' object has no attribute" in str(e):
                # 连接已关闭，尝试重新连接
                self.out("MySQL connection lost during commit. Attempting to reconnect...")
                try:
                    self.__open(force_init=False)
                    self.out("Successfully reconnected to MySQL.")
                    self.__conn.commit()
                    return True
                except Exception as reconnect_error:
                    self.out(f"Failed to reconnect: {reconnect_error}")
                    return False
            return False
        return True
